...
weight: 800001
description: "Clique is a proof-of-authority consensus protocol 以太坊POA共识协议"
title: "EIP225-Clique权威证明共识协议"
...
eip:
  ...
  authorEmail: peterke@gmail.com
  discussionsTo: https://github.com/ethereum/EIPs/issues/225
  status: "Final-完结"
  type: "Standards Track"
  category: Core
  created: 2017-03-06
  source: https://eips.ethereum.org/EIPS/eip-225
---

## 摘要

Clique 是一个权威证明共识协议。
它牵扯以太坊主网设计，因此可不费吹风之力把它能被加入到任何客户端。
...
以太坊的第一个官方测试网络是 Morden。
它从2015年7月运行至2016年11月左右，
当 Geth 和 Parity 之间累积的垃圾和一些 testnet 共识问题时，它最终被搁置以便于太坊测试网重启。
...
Ropsten 测试网就这样诞生了，清除所有垃圾，并从一个干净的状态开始。
它持续运行到2017年2月底，
当恶意行为者决定滥用低PoW， 逐渐将区块Gas限制膨胀到9亿(从正常的4.7亿)，在这刻，发送巨大的交易搞瘫整个网络。
甚至在此之前，攻击者试图进行多次极长的链分叉，导致不同客户端甚至不同版本之间的网络分裂。
...
这些攻击的根本原因是一个 PoW 网络仅靠其背后的计算能力来保障。
从零重启一个新网络不能解决任何事情，因为攻击者可以一遍又一遍地发起同样的攻击。
Parity 团队决定采用紧急解决方案来回滚大量区块，并制定一个不允许 GasLimit 超过某个阈值的软叉规则。
...
* 它不优雅：以太坊应该有动态区块Gas限制
* 它不可移植：其他客户端需要自行实现新的 fork 逻辑
* 它与同步模式不兼容：快和轻客户都不走运
* 它仅延长攻击时间：垃圾交易仍然可以无限制地推进
...
Parity 的解决方案尽管不完美，但仍然可行。
我想提出一个更长期的替代解决方案，它更复杂，但应足够简单，以便在合理的时间内推出。
...
### 标准化的权威证明 Proof-of-authority

如上所述，工作量证明无法在没有价值的网络中安全地工作。
以太坊的长远目标是基于Casper的股权证明，但这是一项繁重的研究，因此我们不能立马靠它它来解决今天的问题。
然而，有一个解决方案很容易实现，但足以有效地修复测试网络，即权威证明方案。
...
这里描述的 PoA 协议的主要设计目标是: 它应该非常简单地实现并嵌入任意现有的以太坊客户端，
同时允许使用现有的同步技术（fast, light, warp），而无需客户端开发人员向关键软件添加自定义逻辑。
...
* 经典方法是从创世块并逐个处理所有交易。这是经过尝试和验证的，但在以太坊复杂网络中，很快计算变得非常昂贵。
* 另一种方法是仅下载区块头并验证其有效性，之后可从网络下载任意最近状态并检查最近的区块头。
...
PoA方案基于以下思想：区块可能仅由可信签名者创建。
因此，客户端看到的每个区块（或区块头）都可以与可信签名者列表进行匹配。
这里的挑战是如何维护可以及时更改的授权签名者列表？
明显的答案（将其存储在以太坊合约中）也是错误的答案：快速，轻快和扭曲同步在同步期间无法访问状态。
fast, light, warp 同步在同步期间无法访问状态。
